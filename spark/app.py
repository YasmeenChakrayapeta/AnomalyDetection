...
CASSANDRA_SERVER = ['35.165.103.197','54.203.47.48','50.112.129.224']


CASSANDRA_NAMESPACE = "ks"
...
app.layout = html.Div([
    html.Div([
        html.H2("Wikipedia Usage Monitor"),
        html.Img(
...
def generate_table(interval):
    news_rec = session.execute('SELECT * FROM Usersflagged')
    materialized_news = list(news_rec)
    df = pd.DataFrame(materialized_news, columns=['global_id', 'Time', 'Username', 'Count'])
    df = df[['Edit-time', 'Username', 'Count']]
    return html.Table(
        # Header
...
def generate_histogram(interval):
    user = session.execute("SELECT username FROM Usersflagged LIMIT 1")
    username = user[0].username
    rows = session.execute("SELECT * FROM usercountsingle WHERE username = \'{}\'".format(username))
...
def gen_wind_speed(interval):
    ...
    rows = session.execute("SELECT count FROM totalInputCountSecond  ")
